...
class Plot_Marching_squares:
    def __init__(self, box, q, color):
        ...
        case = str(self.booltl) + str(self.booltr) + \
            str(self.boolbl) + str(self.boolbr)
        ...
        if case == "0000":
            ...
            plot_line_segment(rightmidlong, rightmidlat,
                              bottommidlong, bottommidlat, color)
        elif case == "0010":
            plot_line_segment(leftmidlong, leftmidlat,
                              bottommidlong, bottommidlat, color)
        elif case == "0011":
            plot_line_segment(leftmidlong, leftmidlat,
                              rightmidlong, rightmidlat, color)
        elif case == "0100":
            plot_line_segment(topmidlong, topmidlat,
                              rightmidlong, rightmidlat, color)
        elif case == "0101":
            plot_line_segment(topmidlong, topmidlat,
                              bottommidlong, bottommidlat, color)
            ...
            plot_line_segment(topmidlong, topmidlat,
                              rightmidlong, rightmidlat, color)
            plot_line_segment(bottommidlong, bottommidlat,
                              leftmidlong, leftmidlat, color)
        elif case == "0111":
            plot_line_segment(topmidlong, topmidlat,
                              leftmidlong, leftmidlat, color)
        elif case == "1000":
...
while (it2 < len(lati)):
    ...
    while (it3 < min):
        # print("Hi")
        ptl = GeoPoint(long[it3-1], lati[it2-1])
        ptr = GeoPoint(long[it3], lati[it2-1])
        pbl = GeoPoint(long[it3-1], lati[it2])
        pbr = GeoPoint(long[it3], lati[it2])

        vtl = data[lati[it2-1]][long[it3-1]]
        vtr = data[lati[it2-1]][long[it3]]
        vbl = data[lati[it2]][long[it3-1]]
        vbr = data[lati[it2]][long[it3]]

        obj = Box(ptl, vtl, ptr, vtr, pbl, vbl, pbr, vbr)
        plotter = Plot_Marching_squares(obj, val, cl)
